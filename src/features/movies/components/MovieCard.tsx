...
const MovieCard = ({ movie }: { movie: Movie }) => {
  ...
  return (
    <div
      className="group relative w-full aspect-[3/2] mobile:aspect-[2/3] rounded-lg overflow-hidden cursor-pointer transition-all duration-300"
    ...
    >
      ...
      <TrailerButton movie={movie} clicked={clicked} />

      <div className="absolute bottom-0 w-full p-3 text-lightest transition-all duration-300">
        <h2 className="text-lg mb-1 truncate max-w-[90%]" title={movie.title}>
          {movie.title}
        </h2>
        ...
        <div
          className={cn(
            "overflow-hidden max-h-0 group-hover:max-h-10 transition-all duration-300 ease-out",
            clicked && "max-h-10"
          )}
        >
